...
import { useRides, MyRide } from '../hooks/useRides';
...
export const HomeScreen = ({ navigation }: any) => {
  ...
  const upcomingRides = myRides.filter(m => !isPast(m.ride.departureTime));
  const completedRides = myRides.filter(m => isPast(m.ride.departureTime));

  const renderRide = ({ ride, role }: MyRide) => {
    const past = isPast(ride.departureTime);
    return (
      <TouchableOpacity key={ride.id} style={styles.rideCard} activeOpacity={0.85}
        onPress={() => navigation.navigate('ConfirmedTrip', { rideId: ride.id })}>
        <View style={styles.rideTop}>
          <View style={styles.rideBadge}>
            <Text style={styles.rideBadgeTxt}>{role === 'driver' ? '🚗 Conductor' : '🙋 Pasajero'}</Text>
          </View>
          {past && (
            <TouchableOpacity style={styles.rateBtn} onPress={() => navigation.navigate('Review', { rideId: ride.id })}>
        ...
        <Text style={styles.rideLink}>Ver viaje ›</Text>
      </TouchableOpacity>
    );
  };
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        <View style={styles.header}>
          <View style={{ flex: 1 }}>
            ...
            <Text style={styles.sub}>{user?.career?.replace(/_/g, ' ')} · ⭐ {user?.rating?.toFixed(1) ?? '—'}</Text>
          </View>
          <TouchableOpacity style={styles.avatar} onPress={() => navigation.navigate('Profile')} activeOpacity={0.85}>
            <Text style={styles.avatarTxt}>{`${user?.name?.[0] ?? ''}${user?.lastName?.[0] ?? ''}`.toUpperCase()}</Text>
          </TouchableOpacity>
        </View>
        ...
        <View style={styles.modeSwitch}>
          ...
          <TouchableOpacity
            style={[styles.modeOpt, isDriver && styles.modeOptActive, !hasVehicle && styles.modeOptLocked]}
            onPress={() => switchMode('driver')}
            activeOpacity={0.8}
          >
            <Text style={[styles.modeOptTxt, isDriver && styles.modeOptTxtActive, !hasVehicle && styles.modeOptTxtLocked]}>
              {hasVehicle ? '🚗 Conductor' : '🔒 Conductor'}
            </Text>
          </TouchableOpacity>
        </View>
        {!hasVehicle && !isDriver && (
          <Text style={styles.lockHintTxt}>🔒 Registra un vehículo para activar el modo conductor.</Text>
        )}
        ...
        {upcomingRides.length > 0 && (
          <>
            <SectionHeader title="Viajes próximos" />
        ...
        {completedRides.length > 0 && (
          <>
            <SectionHeader title="Historial · completados" />
...
const styles = StyleSheet.create({
  ...
  avatar:{width:46,height:46,borderRadius:23,backgroundColor:'#0B1F3A',alignItems:'center',justifyContent:'center',marginLeft:12},
  avatarTxt:{color:'#18A8E0',fontSize:16,fontWeight:'800'},
  ...
  lockHintTxt:{color:'#B7791F',fontSize:12,fontWeight:'600',textAlign:'center',marginTop:-12,marginBottom:16},
  ...
  rideLink:{fontSize:12,color:'#18A8E0',fontWeight:'700',marginTop:8,textAlign:'right'},
